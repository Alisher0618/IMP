...
/*                                                                            */
/*  Reseni vytvoril(a) a odevzdava: Alisher Mazhirinov, xmazhi00              */
/*                                                                            */
/******************************************************************************/
...
void PORTB_IRQHandler(void) {
    ...
    if(PORTB->ISFR & BUTTON_UP_MASK){
    	if(!(GPIOB_PDIR & BUTTON_UP_MASK)){
    		flash();
    		beep();
    	}
    	PORTB->PCR[3] |= PORT_PCR_ISF(0x01);
    }

    if(PORTB->ISFR & BUTTON_DOWN_MASK){
		if(!(GPIOB_PDIR & BUTTON_DOWN_MASK)){
			flash();
			beep();
		}
		PORTB->PCR[6] |= PORT_PCR_ISF(0x01);
	}
